...
IELTS question answering is browser-first, not browser-only. Use Agent API for question-bank data, session metadata, and recent activity when available; always include a browser link so the learner can do the interactive work in IELTS Buddy.
...
For the latest published prediction hits, call `scripts/ielts_buddy_api.py call ielts_prep_search` without a token, with `resourceType="prediction_hit"` and an optional `subject`. Return each record's `practiceUrl` unchanged. It opens the browser's login modal first when needed, then continues to the selected practice start page. Do not add a token or create a session merely to read public prediction data.
...
2. When Agent API is configured, use `ielts_practice_list_taxonomy`, `ielts_practice_search_parts`, `ielts_practice_read_part`, or `ielts_practice_recent_activity` to return a compact data summary: subject, part title, difficulty, tags, available count, recent status, or next launch input.
...
5. If Agent API is unavailable, say the data interface requires the IELTS Buddy Agent API connection and still provide the browser route.
...
- Writing/speaking open-ended answers: do not submit through Agent API for server-side feedback. Use `$ielts-writing-review` or `$ielts-speaking-coach`.
...
- `ielts_practice_submit_session`: use only for objective reading/listening answer-key grading when the user explicitly supplied answers and asked the Agent to submit through Agent API. Do not use it to trigger writing or speaking feedback.
