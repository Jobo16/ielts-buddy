...
| 能力组 | 数据或动作 | 调用约束 |
| --- | --- | --- |
| `ielts_study_plans_list`、`ielts_study_plans_get`、`ielts_study_plans_next_actions` | 读取计划、完整任务和跨计划下一步 | 使用返回的精确 `planId`、任务 ID 与 revision，不猜 ID。 |
| `ielts_courses_search_sections`、`ielts_practice_search_parts`、`ielts_dictation_list_materials`、`ielts_mock_list_papers` | 发现可写入计划的精确内容与稳定 `contentRef` | 内容由调用方选择；服务端只返回事实候选。 |
...
| `ielts_learning_route_read`、`ielts_learning_route_progress`、`ielts_learning_route_next`、`ielts_learner_read_profile`、`ielts_learning_pull_events` | 学习路径、画像和历史事实 | 缺失字段保持未知，不补造数据。 |
