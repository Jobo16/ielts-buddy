...
3. Generate the reviewed DOCX with `../../../scripts/workflows/ielts-task1-review/create_and_validate_task1_review.py` for DOCX input, or `../../../scripts/workflows/ielts-task1-review/create_task1_review_docx.py` followed by the validator for pasted text.
...
1. Identify the prompt, embedded image, and student answer.
   - For `.docx`, extract embedded images with `../../../scripts/workflows/ielts-task1-review/extract_docx_images.py`.
   ...
   - For `.docx`, also run `../../../scripts/workflows/ielts-task1-review/extract_task1_input.py` so the review plan can distinguish prompt text from student answer paragraphs.
...
12. Clean up temporary byproducts after a successful review.
   ...
   - `../../../scripts/workflows/ielts-task1-review/extract_docx_images.py` creates a unique `/private/tmp/ielts_task1_images_*` directory by default; delete that scratch directory after validation.
...
- `../../../scripts/workflows/ielts-task1-review/extract_docx_images.py`: extract visual prompts from new input DOCX files to a unique scratch directory by default.
- `../../../scripts/workflows/ielts-task1-review/extract_task1_input.py`: extract prompt, student answer, and student answer paragraph indices from a Task 1 DOCX.
- `../../../scripts/workflows/ielts-task1-review/create_task1_review_docx.py`: create a reviewed DOCX from a JSON review plan; DOCX input is copied first, comments are anchored into extracted student answer paragraphs, and score/model sections are appended.
- `../../../scripts/workflows/ielts-task1-review/create_and_validate_task1_review.py`: create a reviewed DOCX, validate it, and clean extracted scratch images in one command.
- `../../../scripts/workflows/ielts-task1-review/validate_task1_review_docx.py`: verify comments, italic rewrites, student-answer paragraph comment anchoring when `--input-docx` is supplied, root namespace compatibility, page breaks, font, author, score lines, retained `冲刺 Band 7.5 / 8.0` title, removed old headings, and model answer length/four-paragraph structure.

## JSON Review Plan For DOCX Creation

When using `../../../scripts/workflows/ielts-task1-review/create_task1_review_docx.py`, first produce a JSON file with this shape:
...
```bash
python ../../../scripts/workflows/ielts-task1-review/create_task1_review_docx.py review_plan.json --input-docx MyAnswer.docx --cleanup-plan
python ../../../scripts/workflows/ielts-task1-review/validate_task1_review_docx.py "MyAnswer(reviewed).docx" --input-docx MyAnswer.docx
```

For a DOCX workflow with automatic image scratch cleanup, run:

```bash
python ../../../scripts/workflows/ielts-task1-review/create_and_validate_task1_review.py review_plan.json --input-docx MyAnswer.docx
```
...
- Record locally judged criterion evidence as `rubric` with canonical Writing codes from `../../../scripts/skill-taxonomy.json`; keep band estimates and cited text in event details.
