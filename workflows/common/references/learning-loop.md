...
Use only subjects, subskill codes, and evidence types defined in [skill-taxonomy.json](../../../scripts/skill-taxonomy.json). Missing evidence is `unknown`, never a low score.
...
python3 scripts/learning_store.py snapshot
python3 scripts/learning_store.py next
```
...
```sh
python3 scripts/learning_store.py record-attempt \
  --subject reading \
...
```sh
python3 scripts/learning_store.py record-evidence \
  --subject writing \
  --object-type essay \
  --object-id <essay-id> \
